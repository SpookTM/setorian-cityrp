ENT.Base = "base_ai" -- This entity is based on "base_ai"
ENT.Type = "ai" -- What type of entity is it, in this case, it's an AI.
ENT.AutomaticFrameAdvance = true -- This entity will animate itself.

ENT.PrintName = "Dealer NPC"
ENT.Category = "Setorian Miscellaneous"

ENT.Spawnable = true

function ENT:SetAutomaticFrameAdvance( bUsingAnim ) -- This is called by the game to tell the entity if it should animate itself.
	self.AutomaticFrameAdvance = bUsingAnim
end

function ENT:SetupDataTables()

	self:NetworkVar( "Int", 0, "DealerPrice" )
	self:NetworkVar( "Int", 1, "DespawnTime" )
	self:NetworkVar( "String", 0, "DealerName" )
	self:NetworkVar( "String", 1, "DealerType" )
	self:NetworkVar( "String", 2, "DealerTypeName" )
	self:NetworkVar( "Entity", 0, "LookingForPly" )

end
