...
function PANEL:Init()
	...
	-- setup
	self:SetPadding(ScreenScale(35), false)
	...
	-- self.buttons:SetSize(self:GetWide() * 0.25, ScreenScale(50 / 2))
	self.buttons:SetSize(self:GetWide() * 0.25, 80)
	self.buttons:Dock(TOP)
	-- self.buttons:DockMargin(5, 5, 5, 0)
	...
	close:SetWide(close:GetWide() - 50)
	close:Dock(LEFT)
	close:DockMargin(ScreenScale(50 / 3), ScreenScale(15 / 3), 0, ScreenScale(15 / 3))
	...
	characters:SetWide(characters:GetWide() - 50)
	characters:Dock(RIGHT)
	characters:DockMargin(0, ScreenScale(15 / 3), 0, ScreenScale(15 / 3))
	...
	-- tabs
	self.tabs = self.buttons:Add("DHorizontalScroller")
	self.tabs.buttons = {}
	self.tabs:Dock(FILL)
	self.tabs:SetOverlap( -ScreenScale(12))
	self:PopulateTabs()

	-- function self.tabs.btnLeft:Paint( w, h )
	-- draw.RoundedBox( 0, 0, 0, w, h, Color( 200, 100, 0 ) )
	-- end
	-- function self.tabs.btnRight:Paint( w, h )
...
function PANEL:GetStandardSubpanelSize()
	return ScrW() - self:GetPadding() * 2, ScrH() - self:GetPadding() * 1.5
end
...
function PANEL:SetupTab(name, info, sectionParent)
	...
	-- else
		...
		button:SetWide(button:GetWide() - 50)
		button:Dock(LEFT)
...
function PANEL:PopulateTabs()
	...
	if (IsValid(default)) then
		...
		self.tabs:ScrollToChild(default)
		self.tabs:SetScroll(self.tabs.OffsetX - ScreenScale(200))
	end
...
function PANEL:Remove()
	...
	if IsValid(ix.gui.characterpanelmodel) then
		if ix.gui.characterpanelmodel then
			ix.gui.characterpanelmodel:Remove()
		end
	end
