...
function PANEL:Init()
	...
	self:MoveRightOf(ix.gui.vendor, 8)
	self:MakePopup()
	self:CenterVertical()
	...
	self.storetitle = self:Add("DTextEntry")
	self.storetitle:Dock(TOP)
	self.storetitle:DockMargin(0, 4, 0, 0)
	self.storetitle:SetText(entity:GetTitleStore())
	self.storetitle:SetTall(100)
	self.storetitle:SetMultiline(true)
	self.storetitle:SetPlaceholderText("Store Title")
	self.storetitle.OnEnter = function(this)
		if (entity:GetTitleStore() != this:GetText()) then
		-- 	self:updateVendor("description", this:GetText())
		-- end

			if (!this:GetText() or (string.Trim(this:GetText()) == "" )) then
				this:SetText(entity:GetTitleStore())
				return
			end
	...
	self.storetitle.OnLoseFocus = function( this )
		if (entity:GetTitleStore() != this:GetText()) then
		-- 	self:updateVendor("description", this:GetText())
		-- end
	...
	self.storetitlefont = self:Add("DTextEntry")
	self.storetitlefont:Dock(TOP)
	self.storetitlefont:DockMargin(0, 4, 0, 0)
	self.storetitlefont:SetText(entity:GetTitleFont())
	self.storetitlefont:SetPlaceholderText("Store Title Font (ixMediumFont)")
	self.storetitlefont.OnEnter = function(this)
		if (entity:GetTitleFont() != this:GetText()) then

			if (!this:GetText() or (string.Trim(this:GetText()) == "" )) then
				this:SetText(entity:GetTitleFont())
				return
			end
	...
	self.CanBeRob = self:Add("DCheckBoxLabel")
	self.CanBeRob:SetText("This vendor can be robbed")
	self.CanBeRob:Dock(TOP)
	self.CanBeRob:DockMargin(0, 4, 0, 0)
	self.CanBeRob:SetChecked(entity:GetCanBeRob())
	self.CanBeRob.OnChange = function(this, value)
		-- self:updateVendor("useMoney")
		net.Start("ixVendorTrade_RobEdit")
			net.WriteBool(value)
		net.SendToServer()
	end
