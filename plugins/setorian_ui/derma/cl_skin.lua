...
function SKIN:PaintFrame(panel,width,height)
	...
	if (panel:GetTitle() != "" or panel.btnClose:IsVisible()) then
		...
		draw.RoundedBox(4, 0, 0, width, height, ColorAlpha(color_black, alphaFraction * 20))


	-- surface.SetDrawColor(ColorAlpha(color_white, alphaFraction * 80))
	-- surface.SetTexture(gradient)
...
function SKIN:PaintPanel(panel, width, height, alphaFraction)
	...
	surface.SetDrawColor(ColorAlpha(color_white, alphaFraction * 30))
	surface.SetTexture(gradient)
	...
	-- ix.util.DrawBlur(panel, alphaFraction * 15, nil, 200)
		...
		--    						self.lerpalpha = 15; --15
		-- self.lerpblur = 3;
		-- self.lerptextalpha = 255;
		-- self.lerptextshadow = 100; --100
		-- self.selecty = 0;
		-- self.selectalpha = 0;
		-- self.selected = -1;
		...
		-- 	self.lerptextshadow = Lerp(FrameTime() * 5, self.lerptextshadow, 200); --130

		
        --  local add = 2
		-- local minimum = 160

		-- self.Alpha = math.Clamp((self.Alpha or 255) + add, minimum, 240)


		-- local colour = Color( 0,0,0, self.Alpha)
		-- draw.RoundedBox( 0, 0, 0, width, height, colour )
...
alphaFraction = alphaFraction or 1
	draw.RoundedBox(4, 0, 0, width, height, ColorAlpha(color_black, alphaFraction * 10))
...
do
	...
	sound.Add({
		...
		level = 80,
		pitch = {95, 105},
		sound = bRollover and "ui/scroll_alt_1.wav" or "ui/buttonrollover.wav"
	})
	...
	sound.Add({
		...
		sound = bPress and "ui/select.wav" or "ui/buttonclickrelease.wav"
	})
	...
	sound.Add({
		...
		level = 80,
		pitch = 140,
		sound = bNotify and "ui/info_beep.wav" or "weapons/grenade/tick1.wav"
	})
end
